...
void controlar_sensor(int func) {
	// Ligar Sensor
	if (func == 1) {
		GPIO_SetPinsOutput(BOARD_INITPINS_sensor_ligar_GPIO,
				1 << BOARD_INITPINS_sensor_ligar_PIN);
	}
	// Desligar Sensor
	if (func == 2) {
		GPIO_ClearPinsOutput(BOARD_INITPINS_sensor_ligar_GPIO,
				1 << BOARD_INITPINS_sensor_ligar_PIN);
	}	// Resetar Sensor
	else {
		GPIO_ClearPinsOutput(BOARD_INITPINS_sensor_ligar_GPIO,
...
// Instanciar Sensores
	...
	while (!acc_init) {
		...
		controlar_sensor(3); // Resetar Sensor
		inicializar_I2Cs();
...
int main(void) {
	...
	inicializar_PWMs();
	TPM0->CONTROLS[0].CnV = 1200;
	TPM0->CONTROLS[1].CnV = 4350;
	TPM0->CONTROLS[2].CnV = 5000;
	delay(10000);

	controlar_sensor(1); // Ligar alimentação dos sensores
	inicializar_I2Cs();
